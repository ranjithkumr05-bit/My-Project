// Verify 64 approved subcategories are present in dist bundle
import fs from 'fs'
const dist = fs.readFileSync('dist/assets/index-DDcxnY4S.js', 'utf8')
...
console.log('Total subs entries:', subs.length)
console.log('Unique subs:', unique.length)

const found = subs.filter(s => dist.includes(s))
const missing = subs.filter(s => !dist.includes(s))
console.log('Subs found in bundle:', found.length, '/', subs.length)
