...
const categories = [
  'Corporate & Office Apparel','Sportswear & Team Jerseys','School & College T-Shirts','Workwear & Staff Apparel',
  'Retail & Store Apparel','Hoodies & Sweatshirts','Group & Event T-Shirts','Private Label & Custom Manufacturing'
]
...
const bad = ['safety jacket', 'reflective', 'hi-vis', 'Baniyan & Group', 'Hoodies & Fleece', 'Corporate & Office Polos', 'Sportswear & Jerseys', 'School & College Tees', 'Retail & Staff Tees', 'Private Label Manufacturing']
bad.forEach(w => console.log(`  ${dist.toLowerCase().includes(w.toLowerCase()) ? 'FOUND  ' : 'absent '} ${w}`))

// 'uniform' check — should appear only as the legacy alias key
const uniIdx = []
let ix = dist.toLowerCase().indexOf('uniform')
while (ix !== -1) { uniIdx.push(ix); ix = dist.toLowerCase().indexOf('uniform', ix + 1) }
console.log(`  uniform occurrences: ${uniIdx.length}`)
uniIdx.forEach(i => console.log('    ...' + dist.slice(Math.max(0, i - 55), i + 25).replace(/\s+/g, ' ') + '...'))
