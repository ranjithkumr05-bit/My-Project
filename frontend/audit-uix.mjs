// audit-uix.mjs — UI/UX audit sweep: all routes × 5 viewports, rendered via Playwright.
...
for (const [w, h] of VIEWPORTS) {
  ...
  for (const route of ROUTES) {
    ...
    for (const [k, n] of seen) if (n > 1) add('med', route, vp, 'dup-heading', `${k} ×${n}`)
    ...
    for (let i = 0; i < layout.headerChildren.length; i++) {
      for (let j = i + 1; j < layout.headerChildren.length; j++) {
        ...
        if (a.l < b.r && b.l < a.r) add('high', route, vp, 'header-overlap', `"${a.t}" ∩ "${b.t}"`)
      }
    }
...
// --- navigation integrity: every header/footer link resolves to its own page ---
{
  ...
  for (const { t, h } of links) {
    ...
    if (!KNOWN.has(dest)) { add('high', '/header-footer', '1440x900', 'bad-nav-href', `${t} → ${h}`); continue }
    await page.goto(`${BASE}${h}`, { waitUntil: 'load' })
    await page.waitForTimeout(120)
    const h1 = (await page.locator('h1').first().textContent().catch(() => '')) || ''
    if (!h1.trim()) add('high', '/header-footer', '1440x900', 'nav-no-h1', `${t} → ${h}`)
  }
}
...
// --- CTA flows ---
{
  ...
  if (!(await headerQuote.count())) add('high', '/', '1440x900', 'missing-cta', 'header Get a Quote absent')
    ...
    if (!page.url().includes('/contact?type=quote')) add('high', '/', '1440x900', 'cta-dest', `Get a Quote → ${page.url()}`)
  ...
  if (!page.url().includes('type=sample')) add('high', '/', '1440x900', 'cta-dest', `Request a Sample → ${page.url()}`)
...
console.log('\nReport: audit-uix.report.json | Screenshots: audit-shots/')
