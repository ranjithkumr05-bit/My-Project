...
export function useAsync(fn, deps = []) {
  ...
  useEffect(() => {
    ...
    // `fn` is intentionally not a dependency: callers pass their own `deps` list
    // (see the call sites), and re-running on every render identity change would
    // refetch forever. The spread dep array is this hook's whole contract.
...
export function Badge({value}) {
  const tone = /^(accepted|approved|converted|completed|delivered|won|closed.won|fulfilled)/i.test(
    value,
  )
    ? 'ok'
    : /^(pending|draft|new|open|submitted|in.review)/i.test(value)
      ? 'warn'
      : /^(rejected|lost|cancelled|declined)/i.test(value)
        ? 'bad'
        : /^(quote.sent|approved|active|in.production|sample.requested)/i.test(value)
          ? 'gold'
          : ''
