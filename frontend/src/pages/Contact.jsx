...
export default function Contact() {
  ...
  async function onSubmit(e) {
    ...
    if (!email && !phone) {
      setState({ done: false, busy: false, error: new Error('Add an email or a phone / WhatsApp number so we can send your quotation.') })
      return
    }
  ...
  return (
    <section className="cw-section cw-page">
      <h1>{kind === 'sample' ? 'Request a Sample' : kind === 'quote' ? 'Get a Quote' : 'Contact'}</h1>
      ...
      <div className="cw-grid2">
        {state.done ? (
          ...
          <form className="cw-form" onSubmit={onSubmit}>
            ...
            <p className="cw-hint">Add an email or a phone / WhatsApp number — we need at least one to send your quotation.</p>
            {state.error && <p className="cw-formerr" role="alert">{state.error.message}</p>}
            <button className="btn btn-gold" disabled={state.busy}>{state.busy ? 'Sending…' : kind === 'sample' ? 'Request Sample' : kind === 'quote' ? 'Get a Quote' : 'Send Enquiry'}</button>
          </form>
        )}
        <aside>
