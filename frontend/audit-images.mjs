...
fs.readdirSync(showcase).filter(f => fs.statSync(path.join(showcase, f)).isFile()).forEach(f => console.log('  ' + f))

// Gather image keys referenced in source
const svc = fs.readFileSync('src/data/services.js', 'utf8')
const port = fs.readFileSync('src/pages/Portfolio.jsx', 'utf8')
...
while ((m = reGal.exec(svc))) {
  m[1].split(',').map(s => s.trim().replace(/'/g, '')).filter(Boolean).forEach(k => keys.add(k))
}
